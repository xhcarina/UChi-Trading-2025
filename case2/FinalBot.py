...
data = pd.read_csv('/Users/lydialiuyaqi/Trading/Case2 (1).csv')
...
class MLAllocator():
    def __init__(self, train_data, gb_params=None, pair_params=None):
        ...
        # Initialize a scikit-learn GradientBoostingRegressor for each asset
    ...
    def prepare_ml_data(self, data):
        """Prepare features and targets for the ML model."""
        features = calculate_technical_indicators(data, self.window)
        
        # Use 1-day forward returns as targets
        forward_returns = {}
        ...
        for col in forward_returns:
            y = forward_returns[col].loc[valid_indices].fillna(0)
            y_dict[col] = y
            
        return features, y_dict
    
    def train_models(self):
        """Train models for each asset using ranking targets with a training progress bar."""
        features = calculate_technical_indicators(self.train_data, self.window).fillna(0)
        ...
        # Create ranking targets: long top 2 and short bottom 2 in each period
        ranking_targets = pd.DataFrame(0, index=forward_returns.index, columns=forward_returns.columns)
        for idx in forward_returns.index:
            returns = forward_returns.loc[idx]
            if returns.isna().any():
                continue
        ...
        total_assets = len(self.train_data.columns)
        progress_update_interval = max(total_assets // 5, 1)  # update about every 20%
        
        # Train one model per asset with training progress updates
        for idx, col in enumerate(self.train_data.columns):
            X = features
            y = ranking_targets[col]
            self.models[col].fit(X, y)
            # Print progress update at defined intervals
            if (idx + 1) % progress_update_interval == 0 or (idx + 1) == total_assets:
                percent = (idx + 1) / total_assets * 100
                print(f"Training progress: {percent:.0f}% complete")
    
    def allocate_portfolio(self, current_prices):
        """Allocate portfolio based on model predictions and pair trading signals.
        
        Cleans up technical indicator features by replacing any NaN or infinite values.
        """
        # Update the running price paths with the current prices
        self.running_price_paths = pd.concat([self.running_price_paths,
                                              pd.DataFrame([current_prices])],
                                             ignore_index=True)
        
        # Calculate technical indicators on updated data
        ...
        # Calculate pair trading signal between Asset_4 and Asset_5
...
def grading(train_data, test_data):
    # Design weights to maximize risk-adjusted return (Sharpe ratio)
    weights = np.full((len(test_data.index), 6), fill_value=0.0)
    alloc = MLAllocator(train_data)
    
    total_iterations = len(test_data)
    progress_update_interval = max(total_iterations // 10, 1)  # update every ~10%
    
    print("\nGenerating portfolio weights...")
    for i in range(total_iterations):
        weights[i, :] = alloc.allocate_portfolio(test_data.iloc[i, :])
        # Print a progress update at regular intervals
        if (i + 1) % progress_update_interval == 0 or (i + 1) == total_iterations:
            percent = (i + 1) / total_iterations * 100
            print(f"Allocation progress: {percent:.0f}%")
        if np.sum(weights < -1) or np.sum(weights > 1):
            raise Exception("Weights Outside of Bounds")
    
    capital = [1]  # Start with capital of $1
    print("\nCalculating returns...")
