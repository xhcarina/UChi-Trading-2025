...
class MLAllocator():
    ...
    def train_models(self):
        """Train XGBoost models for each asset using return ranking targets"""
        ...
        # For each time period, rank the assets
        for idx in forward_returns.index:
            # Get returns for this period
            returns = forward_returns.loc[idx]
            # Skip if any returns are NaN
            if returns.isna().any():
                continue
            # Rank assets by return (1 = highest return)
            ranks = returns.rank(ascending=False)
            # Assign targets based on ranks
        ...
        # Train one model for each asset
        for col in self.train_data.columns:
            X = features
    ...
    def optimize_weights(self, predicted_returns, historical_returns):
        ...
        # Test different concentration penalties
        ...
        for penalty in concentration_penalties:
            def neg_sharpe(w):
                ...
                # Add regularization terms with current penalty
                concentration_penalty = -np.sum(w * w) * penalty
                
                # Add drawdown penalty
                drawdown_penalty = -np.max(np.abs(w)) * 0.1  # Penalize large positions
                
                # Calculate risk-adjusted return
                if port_vol != 0:
                    sharpe = port_return / port_vol
                else:
                    sharpe = -1e6  # Penalize zero volatility
        ...
        # Fallback to zero weights if optimization fails
