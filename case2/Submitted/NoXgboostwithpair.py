...
'''
---------------------------------Notes---------------------------------
1. Previous running time record: take 58 seconds to train and 9 mins to run on 9/1 split
...
def calculate_technical_indicators(df, window=30):
    ...
    for i in range(len(cols)):
        for j in range(i+1, len(cols)):
            # Basic spread and ratio
            features_dict[f'spread_{i}_{j}'] = df.iloc[:, i] - df.iloc[:, j]
            features_dict[f'ratio_{i}_{j}'] = df.iloc[:, i] / df.iloc[:, j]
            
            # Cross-asset momentum
            for w in [20, 30]:
                mom_i = df.iloc[:, i].pct_change(periods=w)
                mom_j = df.iloc[:, j].pct_change(periods=w)
                features_dict[f'mom_diff_{i}_{j}_{w}'] = mom_i - mom_j
                features_dict[f'mom_ratio_{i}_{j}_{w}'] = mom_i / mom_j
            
            # Relative strength
            for w in [20, 30]:
                ma_i = df.iloc[:, i].rolling(window=w).mean()
                ma_j = df.iloc[:, j].rolling(window=w).mean()
                features_dict[f'rel_strength_{i}_{j}_{w}'] = ma_i / ma_j
    ...
    for col in df.columns:
        ...
        for w in [20, 30, 60]:
            ...
            features[f'{col}_ema_{w}'] = ema
            features[f'{col}_ema_ratio_{w}'] = prices / ema
        
        # Volatility: standard deviation of returns
        returns = features[f'{col}_returns']
        for w in [20, 30, 60]:
            features[f'{col}_vol_{w}'] = returns.rolling(window=w).std()
            # Add rolling skewness
            features[f'{col}_skew_{w}'] = returns.rolling(window=w).skew()
        
        # Momentum over different windows
        for w in [20, 30, 60]:
            features[f'{col}_mom_{w}'] = prices.pct_change(periods=w)
            # Add rate of change
            features[f'{col}_roc_{w}'] = (prices - prices.shift(w)) / prices.shift(w)
        ...
        for w in [20]:
            ...
            features[f'{col}_bb_upper'] = ma + (2 * std)
            features[f'{col}_bb_lower'] = ma - (2 * std)
            features[f'{col}_bb_width'] = (features[f'{col}_bb_upper'] - features[f'{col}_bb_lower']) / ma
...
class MLAllocator():
    def __init__(self, train_data, gb_params=None, pair_params=None):
        ...
        for col in train_data.columns:
            self.models[col] = GradientBoostingRegressor(
                ...
                subsample=0.8,
                max_features='sqrt',
                validation_fraction=0.1,  # Add validation for early stopping
                n_iter_no_change=5       # Early stopping after 5 iterations
            )
        ...
        # Enhanced pair trading parameters
        self.pair_params = pair_params if pair_params is not None else {
            'z_score_threshold': 2.0,
            'exit_threshold': 0.5,
            'stop_loss_threshold': 3.0,
            'strategy_weight': 0.7,
            'position_scaling': True,     # Enable dynamic position sizing
            'trend_following': True,      # Enable trend following
            'trend_window': 20            # Window for trend calculation
        }
    ...
    def calculate_position_size(self, z_score):
        """Calculate dynamic position size based on z-score."""
        if not self.pair_params['position_scaling']:
            return 1.0
        
        # Scale position size based on z-score magnitude
        base_size = 1.0
        if abs(z_score) > self.pair_params['z_score_threshold']:
            # Increase position size as z-score moves further from threshold
            scale = min(2.0, 1.0 + (abs(z_score) - self.pair_params['z_score_threshold']) / 2.0)
            return base_size * scale
        return base_size
    
    def calculate_trend(self, prices, window=20):
        """Calculate trend strength using moving averages."""
        if not self.pair_params['trend_following']:
            return 0.0
        
        ma_short = prices.rolling(window=window//2).mean()
        ma_long = prices.rolling(window=window).mean()
        trend = (ma_short - ma_long) / ma_long
        return trend.iloc[-1]  # Return latest trend value
    ...
    def allocate_portfolio(self, current_prices):
        ...
        # Calculate trend for pair trading
        trend_4 = self.calculate_trend(self.running_price_paths['Asset_4'])
        trend_5 = self.calculate_trend(self.running_price_paths['Asset_5'])
        trend_diff = trend_4 - trend_5
        ...
        # Enhanced pair trading logic
        if z_score > self.pair_params['z_score_threshold'] and trend_diff < 0:
            pair_weights[3] = -position_size  # Short Asset_4
            pair_weights[4] = position_size   # Long Asset_5
        elif z_score < -self.pair_params['z_score_threshold'] and trend_diff > 0:
